...
beforeAll(() => {
  const link = `<https://api.github.com/user/${username}/repos?page=2&per_page=5>; rel="next", \
                <https://api.github.com/user/${username}/repos?page=23&per_page=5>; rel="last" `;
  const headers = { Link: link };
  fetchMock.get(
    `https://api.github.com/users/${username}/repos?page=1&per_page=5`,
    { body: mockData, headers: headers },
  );
});
